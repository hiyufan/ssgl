...
export const competitionsAPI = {
  ...
  get: async (id: number): Promise<{ competition: Competition; teams_count: number }> => {
    const response = await api.get<{ competition: Competition; teams_count: number }>(
      `/competitions/${id}`
    )
    return response.data
  },
  ...
  recommend: async (): Promise<{
    recommendations: Array<
      Competition & { match_score: number; match_tags: string[]; reason: string }
    >
  }> => {
...
export const milestonesAPI = {
  ...
  batchCreate: async (
    competitionId: number,
    milestones: Array<{ title: string; type: string; due_date: string; sort_order?: number }>
...
export const teamsAPI = {
  ...
  invite: async (teamId: number, userId: number): Promise<{ invitation: TeamInvite }> => {
    const response = await api.post<{ invitation: TeamInvite }>(`/teams/${teamId}/invite`, {
      user_id: userId
    })
    return response.data
  },
...
export const subscriptionsAPI = {
  ...
  ): Promise<{ subscription: CompetitionSubscription }> => {
    const body = remindDaysBefore ? { remind_days_before: remindDaysBefore } : {}
    const response = await api.post<{ subscription: CompetitionSubscription }>(
      `/subscriptions/${compId}`,
      body
    )
    return response.data
  },
  ...
  check: async (
    compId: number
  ): Promise<{ subscribed: boolean; subscription: CompetitionSubscription | null }> => {
    const response = await api.get(`/subscriptions/${compId}/check`)
    return response.data
  },
  reminders: async (): Promise<{
    reminders: Array<{
  ...
  updateSettings: async (
    ...
    const response = await api.put<{ subscription: CompetitionSubscription }>(
      `/subscriptions/${compId}`,
...
export const prePlansAPI = {
  ...
  review: async (id: number): Promise<{ pre_plan: PrePlan; review: Record<string, unknown> }> => {
    const response = await api.post<{ pre_plan: PrePlan; review: Record<string, unknown> }>(
      `/pre-plans/${id}/review`
    )
    return response.data
  }
...
export const awardsAPI = {
  ...
  teacherConfirm: async (id: number): Promise<{ award: Award; action: string }> => {
    const response = await api.post<{ award: Award; action: string }>(
      `/awards/${id}/teacher-confirm`
    )
    return response.data
  },
  settle: async (
...
export const registrationsAPI = {
  ...
  competitionRegistrations: async (
    compId: number,
    params?: Record<string, string>
  ): Promise<{
    registrations: CompetitionRegistration[]
  ...
  register: async (
    compId: number,
    remark?: string
  ): Promise<{ registration: CompetitionRegistration; message: string }> => {
...
export const statsAPI = {
  ...
  recentActivity: async (
    ...
    if (limit) params.limit = String(limit)
    const response = await api.get<{ activities: ActivityItem[]; total: number }>(
...
export const calendarAPI = {
  list: async (
    month?: string
  ): Promise<{ events: CalendarEvent[]; month: string; total: number }> => {
    const response = await api.get<{ events: CalendarEvent[]; month: string; total: number }>(
      '/calendar',
      { params: month ? { month } : {} }
    )
    return response.data
...
export const searchAPI = {
  search: async (
    ...
    const response = await api.get<{ results: SearchResult[]; total: number; query: string }>(
      '/search',
      { params: { q: query } }
    )
    return response.data
...
export const pointsAPI = {
  ...
  leaderboard: async (
    limit?: number
  ): Promise<{ leaderboard: PointsLeaderboardEntry[]; count: number }> => {
    const params: Record<string, string> = {}
    if (limit) params.limit = String(limit)
...
export const notesAPI = {
  listByCompetition: async (
    ...
    const response = await api.get(`/competitions/${compId}/notes`, {
...
export const feedbackAPI = {
  ...
  listByCompetition: async (
    ...
    const response = await api.get(`/competitions/${compId}/feedback`, {
      params: { page, page_size: pageSize }
    })
    return response.data
  },
...
export const ragAPI = {
  query: async (
    ...
    const response = await api.post<{ answer: string; sources: unknown[] }>('/rag/query', {
      question,
      top_k: topK || 5
    })
    return response.data
  ...
  search: async (question: string, topK?: number): Promise<{ results: unknown[] }> => {
    const response = await api.post<{ results: unknown[] }>('/rag/search', {
      question,
      top_k: topK || 5
    })
    return response.data
  ...
  listDocuments: async (
    limit?: number,
    offset?: number
  ): Promise<{ documents: RAGDocument[]; total_chunks: number; total_documents: number }> => {
  ...
  uploadFile: async (
    file: File
  ): Promise<{ message: string; filename: string; chunk_count: number }> => {
  ...
  getDocumentChunks: async (
    ...
    filename: string
    chunks: { id: number; content: string; metadata: unknown; similarity?: number }[]
    total: number
  }> => {
  ...
  batchUpload: async (
    ...
    uploaded: number
    errors: number
    details: { filename: string; chunks: number }[]
  }> => {
    const formData = new FormData()
    files.forEach((f) => formData.append('files', f))
    const response = await api.post('/rag/upload/batch', formData, {
      headers: { 'Content-Type': 'multipart/form-data' }
    })
    return response.data
  }
